...
describe "ActiveRecord American Gladiator" do
  ...
  context "The Maze" do
    it "returns all users that have placed an order" do
      ...
      # Changeable Start
      active_users = User.find(User.joins(:orders).pluck(:user_id))
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      # Changeable Start
      most_popular_item_ids = Item.joins(:order_items).select("items.name, items.id, count(*)").group(:item_id).order("count(*) DESC").limit(2).pluck(:id)
      most_popular_items = Item.find(most_popular_item_ids)
      # Changeable Stop
